...
SUFFIX=""
ARCH=arm
TOOLS_DIR=${PACK_TOPDIR}/sunxi-pack/common/host/bin
export PATH=${TOOLS_DIR}/mod_update:${TOOLS_DIR}/openssl:${TOOLS_DIR}/eDragonEx:${TOOLS_DIR}/fsbuild200:$PATH
...
${BOARD_DIR}/bin/scp.bin:image/scp.fex
${BOARD_DIR}/bin/cpus_pm_binary.code:image/cpus_pm_binary.code
${BOARD_DIR}/bin/optee_${PACK_CHIP}.bin:image/optee.fex
${BOARD_DIR}/bin/semelis_${PACK_CHIP}.bin:image/semelis.bin
${BOARD_DIR}/bin/sboot_${PACK_CHIP}.bin:image/sboot.bin
)
...
function do_prepare()
{
	...
	if [ ! -f ${ROOT_DIR}/image/bootlogo.bmp ]; then
		cp ${ROOT_DIR}/image/boot-resource/bootlogo.bmp ${ROOT_DIR}/image/bootlogo.bmp
		(cd ${ROOT_DIR}/image; ln -sf bootlogo.bmp bootlogo.fex)
	fi

	lzma -k ${ROOT_DIR}/image/bootlogo.bmp
